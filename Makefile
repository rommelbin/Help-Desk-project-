...
composer-install/main-service: generate-dot-env-file
	${DOCKER_COMPOSER} --volume $(shell pwd)/server/main-service:/app ${COMPOSER_COMMAND} install
composer-update/main-service: generate-dot-env-file
	${DOCKER_COMPOSER} --volume ${shell pwd}/server/main-service:/app ${COMPOSER_COMMAND} update

start/server: up/build up